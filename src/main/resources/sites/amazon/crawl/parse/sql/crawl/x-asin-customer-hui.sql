...
select
    ...
    dom_first_text(dom, '#price, #apex_desktop, div[data-feature-name=apex_desktop]') as `price_info`,
    dom_first_text(dom, '#apex_desktop span.basisPrice:contains(List Price)') as `list_price_info`,
    ...
    -- seller info
    cast(dom_all_texts(dom, ' a#sellerProfileTriggerId[href~=seller],' ||
    ...
    str_is_not_empty(dom_first_text(dom, '#acBadge_feature_div span:contains(Amazon), div[data-feature-name=acBadge] span:contains(Amazon)')) as `is_amazon_choice`,
    ...
    cast(dom_all_hrefs(dom, '#availability a, #olp-upd-new-used a, #olp-upd-new a, #certified-refurbished-version a[href~=/dp/], #newer-version a[href~=/dp/]') as varchar) as `sell_same_url`,
    ...
    str_substring_between(dom_first_text(dom, '#olp-upd-new-used a, #olp-upd-new a'), '(', ')') as `other_seller_num`,
    ...
    -- product overview
    cast(dom_all_minimal_htmls(dom, '#productOverview_feature_div table, div[data-feature-name=productOverview] table') as varchar) as `product_overview_html`,
    cast(dom_all_texts(dom, '#productOverview_feature_div table tr, div[data-feature-name=productOverview] table tr') as varchar) as `product_overview`,
    dom_first_text(dom, '#productOverview_feature_div table tr td:contains(Brand) ~ td, div[data-feature-name=productOverview] table tr td:contains(Brand) ~ td') as `product_overview_brand`,
    ...
    -- product information
    -- technical details
    dom_first_minimal_html(dom, 'table#productDetails_techSpec_section_1, table.prodDetTable') as `technical_details_html`,
    dom_first_text(dom, 'table#productDetails_techSpec_section_1, table.prodDetTable') as `technical_details_text`,
    dom_all_texts(dom, 'table#productDetails_techSpec_section_1 tr, table.prodDetTable tr') as `technical_details`,
    dom_first_text(dom, 'table#productDetails_techSpec_section_1 tr th:contains(Brand) ~ td, table.prodDetTable tr th:contains(Brand) ~ td') as `technical_details_brand`,
    dom_first_text(dom, 'table#productDetails_techSpec_section_1 tr th:contains(Manufacturer) ~ td, table.prodDetTable tr th:contains(Manufacturer) ~ td') as `technical_details_manufacturer`,
    dom_first_text(dom, '#prodDetails table tr > th:contains(Product Dimensions) ~ td, #detailBullets_feature_div ul li span:contains(Package Dimensions) ~ span') as `technical_details_detail_volume`,
    dom_first_text(dom, '#prodDetails table tr > th:contains(Item Weight) ~ td') as `technical_details_detail_weight`,

    -- additional information
    dom_first_minimal_html(dom, 'table#productDetails_detailBullets_sections1, #productDetails_db_sections table.prodDetTable') as `additional_info_html`,
    dom_first_text(dom, 'table#productDetails_detailBullets_sections1, #productDetails_db_sections table.prodDetTable') as `additional_info_text`,
    dom_all_texts(dom, 'table#productDetails_detailBullets_sections1 tr, #productDetails_db_sections table.prodDetTable tr') as `additional_info`,
    dom_first_text(dom, 'table#productDetails_detailBullets_sections1 tr th:contains(ASIN) ~ td, #productDetails_db_sections table.prodDetTable tr th:contains(ASIN) ~ td') as `additional_info_asin`,
    dom_first_minimal_html(dom, '#prodDetails table tr > th:contains(Best Sellers Rank) ~ td, #detailBullets_feature_div ul li span:contains(Best Sellers Rank)') as `additional_info_rank_html`,
    dom_first_text(dom, '#prodDetails table tr > th:contains(Best Sellers Rank) ~ td, #detailBullets_feature_div ul li span:contains(Best Sellers Rank)') as `additional_info_rank_text`,
    dom_first_text(dom, '#prodDetails table tr > th:contains(Date First) ~ td, #detailBullets_feature_div ul li span:contains(Date First Available) ~ span') as `additional_info_on_sale_time`,

    cast(dom_all_attrs(dom, '#prodDetails img[src], #productDescription img[src], #dpx-aplus-product-description_feature_div img[src], #dpx-aplus-3p-product-description_feature_div img[src]', 'src') as varchar) as `detail_imgs`,
    cast(dom_all_hrefs(dom, '#rvs-vse-related-videos ol li a[href~=/vdp/]') as varchar) as `detail_videos`,

    dom_first_text(dom, '#prodDetails table tr > th:contains(ASIN) ~ td, #detailBullets_feature_div ul li span:contains(ASIN) ~ span') as `detail_asin`,
    ...
    dom_first_minimal_html(dom, '#aplusBrandStory_feature_div, div[data-feature-name=aplusBrandStory]') as `brand_story_html`,
    dom_first_text(dom, '#aplusBrandStory_feature_div, div[data-feature-name=aplusBrandStory]') as `brand_story_text`,
