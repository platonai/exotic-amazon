...
import ai.platon.exotic.amazon.crawl.boot.component.LinkCollectors
...
class TestLinkCollectors: TestBase() {
    ...
    @Autowired
    private lateinit var hyperlinkCollector: LinkCollectors
    ...
    fun `When collect navigation page then the next page link exists`() {
        ...
        hyperlinkCollector.collectSecondaryLinksFromLabeledPortal(label, page, document, queue)
        assertEquals(1, queue.size)
    }

    @Test
    fun `When create node then the next page link exists`() {
        val node = hyperlinkCollector.updateWebNode(page, document)
        assertNotNull(node)
